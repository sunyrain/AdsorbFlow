"""
一键脚本：为 EqV2 ep187 cfg7_steps5 生成 VASP 输入 + 运行 VASP SP
...
def run_vasp_task(script_dir):
    ...
    try:
        # Use process group so timeout kills ALL children (mpirun + vasp_std)
        ...
        proc.wait()
        cleanup_vasp_files(script_dir)
        return {"dir": script_dir, "success": False, "status": "timeout_1800s"}
...
def main():
    print(f"\n{'='*60}")
    print(f"  VASP SP for EqV2 ep187 cfg7_steps5")
    ...
    # Determine parallelism — leave headroom for GPU training
    total_cores = 64  # cgroup limit
    try:
        ...
        pass
    # Use half the cores to leave room for training + OS
    max_workers = max(1, total_cores // CORES_PER_JOB // 2)
    ...
    with ProcessPoolExecutor(max_workers=max_workers) as executor:
        ...
        for future in tqdm(as_completed(futures), total=len(futures), desc="VASP SP"):
            ...
            if result["success"]:
                ...
                fail_count += 1
                print(f"  FAILED: {result['dir']} - {result['status']}")
