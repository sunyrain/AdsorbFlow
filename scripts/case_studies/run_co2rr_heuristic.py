#!/usr/bin/env python
"""
CO₂RR heuristic baseline (ablation study): skip Flow model entirely.
...
Usage:
    python scripts/run_co2rr_heuristic.py \
        --relax-ckpt configs/relaxation/gemnet_oc/gemnet_oc_base_s2ef_2M.pt \
        --output-dir examples/CO2RR/data_heuristic \
        --device cuda:0
"""
...
# ── CO₂RR adsorbates (same as flow script) ──
CO2RR_ADSORBATES = {
...
def main():
    parser = argparse.ArgumentParser(description="CO₂RR heuristic baseline (ablation)")
    parser.add_argument("--relax-ckpt", required=True)
    parser.add_argument("--output-dir", default="examples/CO2RR/data_heuristic")
    ...
    if not os.path.exists(bulks_path):
        print("ERROR: Run 'python scripts/prepare_co2rr_data.py' first.")
        sys.exit(1)
    ...
    print(f"CO₂RR Heuristic Baseline: {len(bulk_ids)} catalysts × {len(CO2RR_ADSORBATES)} adsorbates")
        ...
        for bulk_id in df["bulk_id"].unique():
            ...
            for ads_name in ["CO", "H", "CHO", "COOH"]:
                ...
                if len(match) > 0:
                    ...
                    if E_slab is not None and not pd.isna(E_slab):
                        row[f"dE_{ads_name}"] = round(E_adslab - E_slab, 3)
            if any(k.startswith("dE_") for k in row):
                summary.append(row)
