#!/usr/bin/env python
"""
OER volcano case study using AdsorbFlow.

For each metal surface, place and relax three adsorbates (*OH, *O, *OOH)
to compute OER descriptor energies and construct a volcano plot.
...
# Free-energy corrections (ZPE + TS) from Nørskov group — kept for reference
# FE_CORRECTIONS = {"OH": 0.35, "O": 0.05, "OOH": 0.40}
# E_H2O = -14.22, E_H2 = -6.77  (not used; GemNet has its own references)
...
def main():
    parser = argparse.ArgumentParser(description="OER volcano with AdsorbFlow")
    ...
    parser.add_argument("--output-dir", default="examples/OER/data_flow")
    ...
    print(f"OER Volcano: {len(bulk_ids)} metals × {len(OER_ADSORBATES)} adsorbates")
    ...
    placement_results = {}  # bulk_id -> {ads_name: adslab_atoms, "slab": slab_atoms}
    ...
    for i, bulk_id in enumerate(bulk_ids):
        ...
        try:
            ...
            if not slab:
                print(f"  SKIP: no slab generated for miller={miller}")
                continue
            slab_atoms = slab[0]
            ...
            for ads_name, ads_info in OER_ADSORBATES.items():
                ...
                # Extract predicted site → recreate adslab
    ...
    torch.cuda.empty_cache()
    gc.collect()
    print(f"\nPlaced {len(placement_results)}/{len(bulk_ids)} metals.")
    ...
    for i, bulk_id in enumerate(bulk_ids):
        ...
        for ads_name, ads_info in OER_ADSORBATES.items():
            ...
            try:
                ...
                # Check anomaly (uses relaxed slab as reference, not initial)
    ...
    if all_energies:
        ...
        # Compute ML adsorption energy descriptors
        # ΔE_ml = E_ml(adslab) - E_ml(bare_slab)
        # GemNet-OC's per-atom reference subtraction means this gives
        # an adsorption energy on an ML-specific scale.
        # We compare trends with literature via R² (like the NRR study).
        ...
        for bulk_id in df["bulk_id"].unique():
            ...
            for ads_name in ["OH", "O", "OOH"]:
                # Prefer non-anomalous; fall back to anomalous for OOH
                # (OOH anomaly detection is overly strict for large adsorbates)
