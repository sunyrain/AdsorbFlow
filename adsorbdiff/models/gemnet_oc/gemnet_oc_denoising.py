...
from adsorbdiff.models.gemnet_oc.initializers import get_initializer
from adsorbdiff.models.gemnet_oc.interaction_indices import (
    get_mixed_triplets,
    get_quadruplets,
    get_triplets,
)
from adsorbdiff.models.gemnet_oc.layers.atom_update_block import OutputBlock
from adsorbdiff.models.gemnet_oc.layers.base_layers import Dense, ResidualLayer
from adsorbdiff.models.gemnet_oc.layers.efficient import BasisEmbedding
from adsorbdiff.models.gemnet_oc.layers.embedding_block import (
    AtomEmbedding,
    EdgeEmbedding,
)
from adsorbdiff.models.gemnet_oc.layers.force_scaler import ForceScaler
from adsorbdiff.models.gemnet_oc.layers.interaction_block import (
    InteractionBlock,
)
from adsorbdiff.models.gemnet_oc.layers.radial_basis import RadialBasis
from adsorbdiff.models.gemnet_oc.layers.spherical_basis import (
    CircularBasisLayer,
    SphericalBasisLayer,
)
from adsorbdiff.models.gemnet_oc.utils import (
    get_angle,
    get_edge_id,
    get_inner_idx,
...
class GemNetOC(BaseModel):
    ...
    def __init__(
        ...
        **kwargs,  # backwards compatibility with deprecated arguments
        ...
        if so3_denoising:
            ...
            self.out_mlp_so3 = torch.nn.Sequential(*out_mlp_F)
            self.out_so3 = Dense(
                emb_size_edge, num_targets, bias=False, activation=None
            )
        ...
        if direct_forces:
            ...
            self.out_so3.reset_parameters(out_initializer)
    ...
    def forward(self, data):
        ...
        if self.so3_denoising:
            x_so3 = self.out_mlp_so3(torch.cat(xs_F, dim=-1))
        with torch.cuda.amp.autocast(False):
            # E_t = self.out_energy(x_E.float())
            if self.direct_forces:
                F_st = self.out_forces(x_F.float())
                so3_st = self.out_so3(x_so3.float())
        ...
        if self.regress_forces:
            if self.direct_forces:
                ...
                so3_st_vec = (
                    so3_st[:, :, None] * main_graph["vector"][:, None, :]
                )
