...
import adsorbdiff
from adsorbdiff.modules.loss import AtomwiseL2Loss, L2MAELoss
...
def _get_project_root() -> Path:
    ...
    root_folder = registry.get("adsorbdiff_root", no_warning=True)

    if root_folder is not None:
        assert isinstance(root_folder, str), "adsorbdiff_root must be a string"
...
def setup_imports(config: Optional[dict] = None) -> None:
    ...
    try:
        ...
        importlib.import_module("adsorbdiff.utils.logger")
...
def check_traj_files(batch, traj_dir) -> bool:
    ...
    traj_files = [traj_dir / f"{id}.traj" for id in batch.sid]
...
def new_trainer_context(*, config: Dict[str, Any], args: Namespace):
    ...
    if TYPE_CHECKING:
        from adsorbdiff.tasks.task import BaseTask
        from adsorbdiff.trainers import BaseTrainer
...
def _resolve_scale_factor_submodule(model: nn.Module, name: str):
    from adsorbdiff.modules.scaling.scale_factor import ScaleFactor
    ...
    try:
        commit_hash = (
            subprocess.check_output(
                ["git", "-C", adsorbdiff.__path__[0], "describe", "--always"]
            )
            .strip()
            .decode("ascii")
        )
