...
from adsorbdiff.placement import Adsorbate, Slab
from adsorbdiff.placement.adsorbate import randomly_rotate_adsorbate
...
class AdsorbateSlabConfig:
    ...
    def get_binding_sites(self, num_sites: int):
        ...
        if self.mode in ["random", "random_site_heuristic_placement"]:
            ...
            num_sites_per_triangle = int(
                np.ceil(2.0 * num_sites / len(simplices))
            )
            for tri in simplices:
        ...
        for site in sites:
            for _ in range(num_augmentations_per_site):
                ...
                metadata_list.append(
                    {"site": site, "xyz_angles": sampled_angles}
                )
    ...
    def _get_scaled_normal(
        ...
        def fun(x):
            return (
                (surf_pos[0] - (cell_center[0] + x * unit_normal[0] + u_[0]))
                ** 2
                + (surf_pos[1] - (cell_center[1] + x * unit_normal[1] + u_[1]))
                ** 2
                + (surf_pos[2] - (cell_center[2] + x * unit_normal[2] + u_[2]))
                ** 2
    ...
    def _find_combos_to_check(
        ...
        pairs = list(
            product(list(range(len(adsorbate_c2))), list(range(len(slab_c2))))
        )

        combos_to_check = []
        for combo in pairs:
            distance = np.linalg.norm(
                projected_points["ads"][combo[0]]
                - projected_points["slab"][combo[1]]
            )
            radial_distance = (
...
def custom_tile_atoms(atoms: ase.Atoms):
    ...
    vectors = [
        v
        for v in atoms.cell
        if ((round(v[0], 3) != 0) or (round(v[1], 3 != 0)))
    ]
...
def get_interstitial_distances(adsorbate_slab_config: ase.Atoms):
    ...
    for combo in pairs:
        ...
        total_distance = np.linalg.norm(
            adsorbate_atoms.positions[combo[1]]
            - surface_atoms.positions[combo[0]]
        )
