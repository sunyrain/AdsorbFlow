...
class Adsorbate:
    ...
    def __init__(
        ...
        if adsorbate_atoms is not None:
            ...
            adsorbate_db = adsorbate_db or pickle.load(
                open(adsorbate_db_path, "rb")
            )
...
def randomly_rotate_adsorbate(
    ...
    if mode == "random":
        ...
        rotvec = np.array(
            [
                np.sqrt(1 - z * z) * np.cos(phi),
                np.sqrt(1 - z * z) * np.sin(phi),
                z,
            ]
        )
        ...
        atoms.rotate(
            a=(0, 0, 1), v=rotvec, center=atoms.positions[binding_idx]
        )
    else:
